...
public class MySQLCreator {
  ...
  private com.bolsinga.music.data.xml.impl.Date createDate(final String sqlDate, final boolean dateVague) throws JAXBException {
  ...
  private void createSongs() throws SQLException, JAXBException {
    ...
    try {
      ...
      while (rset.next()) {
        ...
        String sqlDate = rset.getString("release");
        com.bolsinga.music.data.xml.impl.Date releaseDate = null;
        if (!rset.wasNull()) {
          boolean dateVague = (rset.getInt("release_vague") == 1);
